...
パッケージの更新にあわせて、アセットフォルダーに展開したデータが更新された可能性があります。以下の手順で更新できます。

1. Gitなどのバージョン管理をしていたらコミットします
1. UnityのProjectウィンドウで、Packages > AM1 Base Framework > Package Resourcesフォルダーを開きます
1. BaseFrameパッケージをダブルクリックします
1. 更新するファイルを選択してImportします

場合によって、修正した変更が上書きされる可能性があります。その場合はGitなどのバージョン管理で対応してください。
...
<s>
本システムでは、シーン内のいずれかのゲームオブジェクトのAwake()で`SceneStateChanger.AwakeDone(gameObject.scene.name);`を呼ぶことでシーンの読み込みが完了したことをシステムに知らせる必要があります。 </s> この仕様は0.9.4で不要になりました。
